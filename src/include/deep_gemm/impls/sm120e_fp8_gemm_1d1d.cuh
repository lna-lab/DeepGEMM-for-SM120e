// SM120e port of DeepGEMM's sm100_fp8_gemm_1d1d kernel.
//
// Delta from SM100:
//   - tcgen05_after_thread_sync()  → __syncthreads()   (3 sites)
//   - tcgen05_before_thread_sync() → __syncthreads()   (1 site)
//   - Function renamed to sm120e_fp8_gemm_1d1d_impl
//   - Error message updated for SM120
//
// SM120e constraints:
//   - Shared memory: 49,152 bytes (vs SM100's 168 KB)
//   - Instantiate with BLOCK_M=64, BLOCK_N=64, kNumStages=1
//   - tcgen05.mma / TMA / UMMA / TMEM all work on SM120
//   - Only tcgen05.fence is unsupported → replaced with __syncthreads()
...
sm120e_fp8_gemm_1d1d_impl(int* grouped_layout,
                          uint32_t shape_m, uint32_t shape_n, uint32_t shape_k,
    ...
    if (warp_idx == 0 and cute::elect_one_sync()) {
        ...
        while (scheduler.get_next_block(m_block_idx, n_block_idx)) {
            ...
            // SM120e: replaced tcgen05_after_thread_sync() with __syncthreads()
            __syncthreads();
            ...
            for (uint32_t k_block_idx = 0; k_block_idx < num_total_k_blocks; advance_pipeline(k_block_idx)) {
                ...
                // SM120e: no explicit tcgen05.fence::before_thread_sync needed here,
                // as tcgen05.commit performs it implicitly (same as SM100)
        ...
        while (scheduler.get_next_block(m_block_idx, n_block_idx)) {
            ...
            for (uint32_t w = 0; w < kNumMWaves; ++ w) {
                ...
                for (uint32_t s = 0; s < kNumStores; ++ s, advance_store_pipeline()) {
                    ...
                    // Notify tensor memory empty arrival
                    if (w == kNumMWaves - 1 and s == BLOCK_N / STORE_BLOCK_N - 1) {
                        // SM120e: replaced tcgen05_before_thread_sync() with __syncthreads()
                        __syncthreads();
    ...
    if (blockIdx.x == 0 and threadIdx.x == 0)
        DG_DEVICE_ASSERT(false and "This kernel requires sm_120 or later");
#endif
}
